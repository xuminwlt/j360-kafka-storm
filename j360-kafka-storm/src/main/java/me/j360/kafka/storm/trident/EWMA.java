...
public class EWMA implements Serializable {
    ...
        public EWMA() {
            }
     
                public EWMA sliding(double count, Timetime) {
    ...
     
        ...
                if (!(alpha > 0.0D && alpha<= 1.0D)) {
                        throw newIllegalArgumentException("Alpha must be between 0.0 and 1.0");
                    }
                this.alpha = alpha;
                return this;
            }
        public EWMA withAlphaWindow(longalphaWindow) {
                this.alpha = -1;
